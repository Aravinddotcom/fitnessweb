...
import { DATE_FORMATS_OUT } from '../formats/date-formats';
...
export class InvoiceComponent implements OnInit {
  ...
  handleSyncButton(drawer: { close: () => void; }) {
    this.isDirty = false;
  ...
  handlePicker(pickerDate: string) {
    this.invoice.dueDate = pickerDate.replace(/\//g, '-');
    this.displayDate = this.invoice.dueDate.split('-').reverse().join('-');
    this.isDirty = true;
  }

  openDialog(drawer: { close: () => void; }) {
