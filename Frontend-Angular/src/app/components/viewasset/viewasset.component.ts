...
export class ViewassetComponent implements OnInit {
  ...
  currentMfPrice:AllMutualFund={mutualFundId:0,foodName:"",caloriesValue:0};
  currentStockPrice:AllStock={stockId:0,productName:"",proteinValue:0}
  ...
  updateMutualFundList()
  {
    ...
    for(let i=0;i<this.mutualFundl.length;i++){
      this.currentMfPrice={mutualFundId:0,foodName:"",caloriesValue:0};
      this.token=  this.loginService.getToken();
      if(this.token!=null)
      {
        this.mfservice.getCurrentMutualFund(this.mutualFundl[i].foodName).subscribe((data:AllMutualFund)=>
        {
          console.log(data);
          if(this.mutualFundl[i].foodName.match(data.foodName)){

            this.mutualFundl[i].currentPrice=data.caloriesValue;
          }
        },
        (error:any)=>
        {
  ...
  updateStockDetailList()
  {
    ...
    for(let i=0;i<this.stockDetailList1.length;i++){
      this.currentStockPrice={stockId:0,productName:"",proteinValue:0}
      this.token=  this.loginService.getToken();
      if(this.token!=null)
      {
        this.sdService.getStockDetails(this.stockDetailList1[i].productName).subscribe((data:AllStock)=>
        {
          console.log(data);
          if(this.stockDetailList1[i].productName.match(data.productName)){
            this.stockDetailList1[i].currentPrice=data.proteinValue;
          }
        },
        (error:any)=>
        {
